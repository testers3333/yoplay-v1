...
module.exports = {
    ...
    async run(client, interaction) {
        ...
        readdirSync("./src/commands").forEach((folder) => {
            readdirSync(`./src/commands/${folder}`).forEach(async (file) => {
                let d = require(`../../commands/${folder}/${file}`);
                ...
                if(d.name === interaction.commandName) {
                    ...
                    if(d.cooldown && db.get("cooldown-"+d.name+"-"+interaction.user.id) && db.get("cooldown-"+d.name+"-"+interaction.user.id) > parseInt(Date.now() / 1000)) {
                        return interaction.editReply({
                            embeds: [
                                new MessageEmbed()
                                .setColor("#ed4147")
                                .setDescription("Merci d'attendre encore quelques temps avant de réutiliser la commande. Revenez <t:"+ db.get("cooldown-"+d.name+"-"+interaction.user.id) +":R> !")
                                .setFooter({
                                    text: "401 - Manque d'authorization",
                                    iconURL: "https://cdn.discordapp.com/emojis/1013206759873192077"
                                })
                   ...
                   db.set("cooldown-"+d.name+"-"+interaction.user.id, parseInt(Date.now() / 1000) + d.cooldown);
                    return d.run(client, interaction).catch((e) => {
                        ...
                        client.channels.cache.get(process.env["error_channel"]).send({
                            ...
                            components: [
                                ...
                                .addComponents(
                                    new MessageButton()
                                    .setCustomId("corrected."+folder+"."+file+"."+true)
                                    .setLabel("Corrigé ?")
                                    .setStyle("DANGER")
                                    .setDisabled(false)
                                )
